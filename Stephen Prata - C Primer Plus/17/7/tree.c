...
unsigned FindInTree(const Tree* tree, const Item* item) {

	const Node* temp = recursiveSearch(tree->root, item);
...
bool AddToTree(Item* item, Tree* tree) {
	...
	else if ((tempNode = recursiveSearch(tree->root, item)) != NULL) {
		(tempNode->item).count++;
		return false;
	}
...
static bool toLeft(const Item* item1, const Item* item2) {
	...
	if(strcmp(item1->word, item2->word) < 0) return true;
	else return false;
...
static Node* recursiveSearch(Node* node, const Item* item) {
	...
	else if (isEqual(item, &node->item)) return node;
...
static void copyToNode(Node* node, const Item* item) {
	...
	strcpy((node->item).word, item->word);
	(node->item).count = 1;
